# tools/build-blog-pdf.ps1
#
# Renders docs/blog/power-platform-tenant-inventory-in-60-seconds.md to a
# single self-contained PDF using Microsoft Edge's headless print engine.
#
# Pipeline:
#   1. Read the markdown.
#   2. Convert to HTML with a tiny in-house renderer (no third-party
#      dependencies — keeps the doc-build path zero-install).
#   3. Inline-base64 every local image reference so the headless print
#      doesn't need a local web server.
#   4. Inject mermaid.min.js (Edge fetches it from the CDN once during
#      print) so the architecture diagram renders inline.
#   5. Write the HTML to %TEMP%, then run msedge.exe --headless
#      --print-to-pdf= against it.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\tools\build-blog-pdf.ps1
#
# Output: docs/blog/power-platform-tenant-inventory-in-60-seconds.pdf
...
# Resolve defaults relative to the script file (PS 5.1 evaluates $PSScriptRoot
# as $null in param defaults when invoked via -File, so we resolve here).
$scriptRoot = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
if (-not $InMd)   { $InMd   = Join-Path $scriptRoot '..\docs\blog\power-platform-tenant-inventory-in-60-seconds.md' }
if (-not $OutPdf) { $OutPdf = Join-Path $scriptRoot '..\docs\blog\power-platform-tenant-inventory-in-60-seconds.pdf' }
$InMd   = (Resolve-Path $InMd).Path
$OutPdf = [IO.Path]::GetFullPath($OutPdf)
if (-not (Test-Path $EdgeExe)) { throw "Edge not found at $EdgeExe" }
if (-not (Test-Path $InMd))    { throw "Markdown not found at $InMd" }

$mdDir = Split-Path -Parent (Resolve-Path $InMd).Path
# Read as UTF-8 explicitly. Get-Content -Raw on PS 5.1 defaults to the system
# code page, which mangles em dashes (U+2014) into "â€"" mojibake. Use the
# .NET API to lock the encoding regardless of host.
$md    = [IO.File]::ReadAllText($InMd, [Text.Encoding]::UTF8)

# ---------------------------------------------------------------------
# 1) Inline images. Convert ![alt](path) -> ![alt](data:image/png;base64,...)
#    so the print engine doesn't have to resolve relative paths or hit
#    the network.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 2) Convert markdown -> HTML. Hand-rolled, deliberately tiny: covers
#    h1/h2/h3, paragraphs, bold/italic, inline code, links, images,
#    bullet lists, ordered lists, blockquotes, hr, fenced code blocks
#    (incl. mermaid passthrough), and pipe tables. That's everything
#    the blog uses.
# ---------------------------------------------------------------------
function Convert-MarkdownToHtml {
    ...
    function Inline {
        ...
        $s = $s -replace '<', '&lt;'
        $s = $s -replace '>', '&gt;'
        # images ![alt](src) — keep before links; alt may contain quotes, so escape them
        $s = [regex]::Replace($s, '!\[([^\]]*)\]\(([^)]+)\)', {
            param($m)
        ...
        # italic *x* (avoid touching ** that already became <strong>)
    ...
    while ($i -lt $lines.Count) {
        ...
        if ($line -match '^```(?<lang>\w*)\s*$') {
            ...
            if ($lang -eq 'mermaid') {
                # passthrough — mermaid.js will render this client-side
...
# ---------------------------------------------------------------------
# 3) Wrap in print-friendly HTML with mermaid.js for diagram rendering.
# ---------------------------------------------------------------------
$css = @'
@page { size: Letter; margin: 0.6in 0.55in; }
body { font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; color: #1f1f1f; font-size: 10.5pt; line-height: 1.45; max-width: 100%; margin: 0; }
...
<script>
  // Render mermaid diagrams synchronously so headless print waits for SVG.
  mermaid.initialize({ startOnLoad: false, theme: 'neutral', securityLevel: 'loose', flowchart: { htmlLabels: true, useMaxWidth: true } });
  mermaid.run().then(function(){ document.title = 'VerseOps blog (rendered)'; }).catch(function(e){ console.error(e); });
</script>
</body>
</html>
"@

$tmpHtml = Join-Path $env:TEMP "verseops-blog-$(Get-Date -Format 'yyyyMMddHHmmss').html"
[IO.File]::WriteAllText($tmpHtml, $html, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "wrote $tmpHtml ($([math]::Round((Get-Item $tmpHtml).Length/1KB,1)) KB)"

# ---------------------------------------------------------------------
# 4) Print to PDF via headless Edge. We use Start-Process (instead of
#    the call operator) because the call operator's stdout/stderr
#    redirection caused Edge to terminate before producing the PDF on
#    this host. --no-pdf-header-footer suppresses the default URL/page
#    footer; --virtual-time-budget gives mermaid time to render.
# ---------------------------------------------------------------------
if (Test-Path $OutPdf) { Remove-Item $OutPdf -Force }
$edgeProfile = Join-Path $env:TEMP ("verseops-edge-" + [guid]::NewGuid())
New-Item -ItemType Directory -Path $edgeProfile -Force | Out-Null
$edgeLog = Join-Path $env:TEMP "verseops-edge.log"
$edgeArgs = @(
    '--headless',
    '--disable-gpu',
...
Write-Host "msedge: $EdgeExe"
Write-Host "args  : $($edgeArgs -join ' ')"
$p = Start-Process -FilePath $EdgeExe -ArgumentList $edgeArgs -Wait -NoNewWindow -PassThru -RedirectStandardError $edgeLog
Write-Host "exit code: $($p.ExitCode)"
Remove-Item -Recurse -Force $edgeProfile -ErrorAction SilentlyContinue
Remove-Item $edgeLog -ErrorAction SilentlyContinue

if (Test-Path $OutPdf) {
    Write-Host "OK -> $OutPdf ($([math]::Round((Get-Item $OutPdf).Length/1KB,1)) KB)"
} else {
    throw "PDF was not produced at $OutPdf (exit=$($p.ExitCode))"
}
