...
![VerseOps empty-state dashboard — the FluentWindow shell, hero KPI tiles, refresh toolbar, and grid header before any tenant data is loaded.](images/01-empty-dashboard.png)
*Figure 1 — VerseOps on first launch, before sign-in. Single-window WPF FluentWindow with Mica backdrop. Five hero KPI tiles (Environments / Tenant Database / Tenant File / Total Assets / Licensed Users) and a paginated environments grid with filterable columns.*
